...
def SIGNAL_PEAK_TO_PEAK_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: float = 0.0, error_spread=0.0):
...
def SIGNAL_RMS_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: float = 0.0, error_spread=0.0):
...
def SIGNAL_MEAN_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: float = 0.0, error_spread=0.0):
    hardware_available, measured_value = apply_signal_measure(g, signal, reference, 'signal_mean_measure', expected_value=expected_value)
    if not hardware_available:
        return expected_value + random.uniform(-error_spread, error_spread)
...
def SIGNAL_MIN_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: float = 0.0, error_spread=0.0):
...
def SIGNAL_MAX_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: float = 0.0, error_spread=0.0):
...
def SIGNAL_HIGH_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: float = 0.0, error_spread=0.0):
    hardware_available, measured_value = apply_signal_measure(g, signal, reference, 'signal_high_measure', expected_value=expected_value)
    if not hardware_available:
        return expected_value + random.uniform(-error_spread, error_spread)
    g.output.append({'type': 'MEASURE', 'measurement': 'signal_high', 'signal': signal, 'reference': reference, 'measured_value': measured_value})
    return measured_value

def SIGNAL_LOW_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: float = 0.0, error_spread=0.0):
    hardware_available, measured_value = apply_signal_measure(g, signal, reference, 'signal_low_measure', expected_value=expected_value)
    if not hardware_available:
        return expected_value + random.uniform(-error_spread, error_spread)
    g.output.append({'type': 'MEASURE', 'measurement': 'signal_low', 'signal': signal, 'reference': reference, 'measured_value': measured_value})
    return measured_value

def SIGNAL_AMPLITUDE_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: float = 0.0, error_spread=0.0):
    hardware_available, measured_value = apply_signal_measure(g, signal, reference, 'signal_amplitude_measure', expected_value=expected_value)
    if not hardware_available:
        return expected_value + random.uniform(-error_spread, error_spread)
...
def SIGNAL_POSITIVE_PULSE_COUNT_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: int = 0, error_spread=0):
    hardware_available, measured_value = apply_signal_measure(g, signal, reference, 'signal_positive_pulse_count_measure', expected_value=expected_value)
    if not hardware_available:
...
def SIGNAL_NEGATIVE_PULSE_COUNT_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: int = 0, error_spread=0):
    hardware_available, measured_value = apply_signal_measure(g, signal, reference, 'signal_negative_pulse_count_measure', expected_value=expected_value)
    if not hardware_available:
        return expected_value
...
def SIGNAL_RAISING_EDGE_COUNT_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: int = 0, error_spread=0):
    hardware_available, measured_value = apply_signal_measure(g, signal, reference, 'signal_raising_edge_count_measure', expected_value=expected_value)
    if not hardware_available:
        return expected_value
...
def SIGNAL_FALLING_EDGE_COUNT_MEASURE(signal: str = 'VCC', reference: str = 'GND', expected_value: int = 0, error_spread=0):
    hardware_available, measured_value = apply_signal_measure(g, signal, reference, 'signal_falling_edge_count_measure', expected_value=expected_value)
    if not hardware_available:
        return expected_value
